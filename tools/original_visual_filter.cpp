...
void printUsage(const char *program) {
  std::cerr
      ...
      << "  --check                  Validate support and allocation, then exit\n"
...
bool writeStats(const Options &options, std::span<const double> kernelFrameTimes,
  ...
  const bool hasCertificationEvidence =
      ...
      warmupFrames == kCertificationWarmupFrames &&
  ...
  output << std::fixed << std::setprecision(3) << "{\n"
         ...
         << "    \"cdf97_fwt_and_wpt_supported_other_wavelets_and_predictor_search_modes_rejected\"\n"
         << "  ],\n"
         << "  \"width\": " << options.width << ",\n"
         ...
         << "  \"stream_wall_timing_scope\": \"post-warmup-pre-read-through-completed-write\",\n"
         << "  \"all_frames_mean_process_ms\": "
         << allKernel.meanMilliseconds << ",\n"
         << "  \"all_frames_stream_wall_mean_ms\": "
         << allStream.meanMilliseconds << ",\n"
         ...
         << "    \"scope\": \"filter-process ingress-to-egress including pipe wait/backpressure; final mux excluded\"\n"
         << "  },\n"
...
int main(int argc, char **argv) {
  ...
  std::string error;
  if (!lane.prepare(options.width, options.height, config, error)) {
  ...
  if (std::setvbuf(stdin, inputIoBuffer.data(), _IOFBF,
                   inputIoBuffer.size()) != 0 ||
  ...
  kernelFrameTimes.reserve(65536);
  streamFrameTimes.reserve(65536);
  ...
  while (true) {
    ...
    if (bytesRead != frameBytes) {
      std::cerr << "Incomplete BGRA frame at index "
                << kernelFrameTimes.size()
                << ": got " << bytesRead << " of " << frameBytes
                << " bytes\n";
      return 5;
    }

    glic::OriginalRealtimeFrameStats frameStats;
    const auto kernelStart = std::chrono::steady_clock::now();
    if (!lane.process(input, output, &frameStats, error)) {
      std::cerr << "Frame " << kernelFrameTimes.size()
                << " failed: " << error << '\n';
      return 5;
    }
    const auto kernelFinish = std::chrono::steady_clock::now();
    kernelFrameTimes.push_back(std::chrono::duration<double, std::milli>(
                                   kernelFinish - kernelStart)
                                   .count());
    for (std::size_t channel = 0; channel < segmentTotals.size(); ++channel)
      segmentTotals[channel] += frameStats.segmentCounts[channel];
    ...
    if (bytesWritten != frameBytes || std::fflush(stdout) != 0) {
      std::cerr << "Failed to write BGRA frame "
                << kernelFrameTimes.size() - 1
                << '\n';
      return 6;
    }
    const auto streamFinish = std::chrono::steady_clock::now();
    streamFrameTimes.push_back(std::chrono::duration<double, std::milli>(
                                   streamFinish - streamStart)
                                   .count());
  }
  ...
  std::cerr << "frames=" << kernelFrameTimes.size()
            << " backend=cpu-original-visual preset=" << options.preset
