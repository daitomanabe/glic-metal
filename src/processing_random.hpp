...
public:
  ...
  // GLIC's quadtree sizes are powers of two. Callers that already proved that
  // invariant can hoist countr_zero() out of a sampled block and preserve the
  // exact Processing random(float(size)) result with one 24-bit RNG step.
  int nextPowerOfTwoPosition(unsigned magnitude) noexcept {
    if (magnitude > 24u)
      return 0;
    const std::uint32_t bits = nextBits(24);
    return static_cast<int>(bits >> (24u - magnitude));
  }
...
// A process call is transactional with respect to its evolving segmentation
// stream: failed frames must not perturb the tree generated by the next
// successful frame. The checkpoint rolls back unless commit() is reached.
class ProcessingRandomCheckpoint {
public:
  explicit ProcessingRandomCheckpoint(ProcessingRandom &random) noexcept
      : random_(random), saved_(random) {}

  ProcessingRandomCheckpoint(const ProcessingRandomCheckpoint &) = delete;
  ProcessingRandomCheckpoint &
  operator=(const ProcessingRandomCheckpoint &) = delete;

  ~ProcessingRandomCheckpoint() noexcept {
    if (!committed_)
      random_ = saved_;
  }
...
private:
  ProcessingRandom &random_;
  ProcessingRandom saved_;
