...
| `codec_glitch` | H.264の時間参照・量子化・encode済みframe cadence・decode済みframe履歴を使う動画グリッチ | 下記18 effect、GLIC presetとは無関係 |
...
書き換えません。AV2 motion refinementを前提とするOptical Flow Woundは、H.264
VideoToolboxの公開APIに同等のhookがないため追加していません。
...
- `slice_dropout`、`slice_transplant`、`payload_xor`、
  `reference_timewarp`と研究版6 effectはclean decode後の`CVPixelBuffer`、または
...
18 effect名は`--codec-effect`へそのまま指定できます。このモードはmacOSと
...
全18 effectを異なる強度、時間周期、feedback、seedで展開した最大50候補をまとめて
...
| `codec_glitch` | Video artifacts based on H.264 prediction, quantization, encoded-frame cadence, and decoded-frame history | 18 effects below; no GLIC preset semantics |
...
refinement has no equivalent hook in the public H.264 VideoToolbox API.
...
- `slice_dropout`, `slice_transplant`, `payload_xor`, `reference_timewarp`, and
  the six research effects act only on clean-decoded `CVPixelBuffer` output or its
...
All eighteen names in the table are accepted by `--codec-effect`. Codec mode
