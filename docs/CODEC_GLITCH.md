...
| `codec_glitch` | H.264の時間参照・量子化・encode済みframe cadence・decode済みframe履歴を使う動画グリッチ | 下記28 effect、GLIC presetとは無関係 |
...
[CODEC_LAB.md](CODEC_LAB.md)に分離しています。
...
- `slice_dropout`、`slice_transplant`、`payload_xor`、
  `reference_timewarp`と研究版6 effect、Realtime Crossbreed 10 effectはclean decode後の`CVPixelBuffer`、または
...
28 effect名は`--codec-effect`へそのまま指定できます。このモードはmacOSと
...
全28 effectを異なる強度、時間周期、feedback、seedで展開した最大50候補をまとめて
...
| `codec_glitch` | Video artifacts based on H.264 prediction, quantization, encoded-frame cadence, and decoded-frame history | 28 effects below; no GLIC preset semantics |
...
- `slice_dropout`, `slice_transplant`, `payload_xor`, `reference_timewarp`, and
  the six research effects and ten Realtime Crossbreed effects act only on clean-decoded `CVPixelBuffer` output or its
...
All 28 names in the table are accepted by `--codec-effect`. Codec mode
