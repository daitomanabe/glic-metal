...
| `codec_glitch` | H.264の時間参照・量子化・encode済みframe cadence・decode済みframe履歴を使う動画グリッチ | 下記12 effect、GLIC presetとは無関係 |
...
6. `slice_dropout`、`slice_transplant`、`payload_xor`、
   `chroma_codec_echo`、縮小復元／pixel化はMetal互換pixel-buffer poolと
...
- `slice_dropout`、`slice_transplant`、`payload_xor`、
  `reference_timewarp`はclean decode後の`CVPixelBuffer`、またはその有界履歴だけへ
  作用します。圧縮VCL sliceの削除・移植、payload XOR、過去P packetの再投入は
  行いません。
...
12 effect名は`--codec-effect`へそのまま指定できます。このモードはmacOSと
...
| `codec_glitch` | Video artifacts based on H.264 prediction, quantization, encoded-frame cadence, and decoded-frame history | 12 effects below; no GLIC preset semantics |
...
5. All other H.264 packets reach the VideoToolbox hardware decoder without
   byte modification.
6. `slice_dropout`, `slice_transplant`, `payload_xor`, `chroma_codec_echo`, and
   scaled/pixelated recovery use Metal-compatible pixel-buffer pools and a
...
- `slice_dropout`, `slice_transplant`, `payload_xor`, and
  `reference_timewarp` act only on clean-decoded `CVPixelBuffer` output or its
...
All twelve names in the table are accepted by `--codec-effect`. Codec mode
