...
| Native Compressed Syntax Lab | 12 / 16 variants | FFglitch MPEG-2 `mv` / `q_dct` / `qscale`、MPEG-4 Part 2 `mv` | 非対応 |
...
保持します。H.264/HEVC entropy syntaxの直接編集は未実装でfail-closedします。
詳細は[NATIVE_SYNTAX_GLITCH.md](NATIVE_SYNTAX_GLITCH.md)を参照してください。
`evaluate_native_syntax_glitches.py`は16 codec-effect variantを一括処理し、実動画
...
MV effects on MPEG-4 Part 2. Its token-free batch evaluator ranks all 16
codec-effect variants using actual-video difference and diversity.
H.264/HEVC direct entropy editing fails closed. AV2 processing uses the pinned
