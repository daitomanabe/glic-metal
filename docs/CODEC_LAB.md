...
| Native Compressed Syntax Lab | 8 | FFglitch MPEG-2 `mv` / `q_dct` transplication | 非対応 |
...
（`mv`）または量子化DCT係数（`q_dct`）をFFglitch `ffedit`でexportし、値を
直接変更してentropy syntaxへtransplicateします。既存Syntax Labの
decoded reconstruction proxyとは別workflowです。
...
syntax effects, and 11 structured/transport/metadata operations.
...
quantized DCT coefficients directly. H.264/HEVC direct entropy editing fails
closed. AV2 processing uses the pinned official AVM tools and fails closed
when unavailable.
