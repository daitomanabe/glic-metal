...
- `codec-lab-effects.json` — realtime/offline codec-lab capability catalog.
- `docs/CODEC_LAB.md` — Syntax Lab、analysis、token-free search contract.
- `docs/GLITCH_EXPANSION.md` — 追加全系統の実装レベルと実動画評価。
- `docs/DOWNSTREAM_QUICKSTART.md` — shortest downstream setup path.
...
| Codec Glitch | 36 codec effects | `CVPixelBufferRef` | Stateful H.264 / HEVC / ProRes encode/decode and codec-history effects |
| Offline Packet Lab | 8 packet effects | video file | Isolated compressed-stream damage and salvage decode |
| Offline Syntax / Analysis Lab | 21 workflows | video file | Real codec cycles, reconstruction, analysis, and search |
...
AV1, AV2, VP9, VVC, Theora, and Dirac are offline/reference workflows rather than members of this
...
Packet mutation is also outside the realtime C ABI. Invoke
`scripts/process_offline_packet_glitch.py` in an isolated child process; never
decode its damaged stream inside the host. Check
`resources/offline-codec-effects.json` before launch and consume the preview
plus JSON report. Unequal frame counts are expected, so compare outputs with
`scripts/evaluate_offline_packet_glitches.py`. See
[OFFLINE_PACKET_GLITCH.md](OFFLINE_PACKET_GLITCH.md).

Syntax reconstruction、multi-decoder analysis、cross-codec chains、token-free
search are also offline-only. Launch `scripts/process_codec_lab.py` or
`scripts/evolutionary_codec_search.py`, validate names through
...
Codec Glitchは、上記の同期画像APIとは独立した、VideoToolboxによる非同期codec
...
controls.rate = 0.34f;
const char *implementation_level =
    glic_codec_glitch_effect_implementation_level(controls.effect);
...
The generated SDK is self-contained. `Documentation/` contains these
integration contracts and `Tools/` contains the offline codec/packet/search
entrypoints plus `requirements.txt`. CMake installs expose the same entrypoints
through `GLIC_METAL_TOOLS_DIR` and their Python dependency file through
