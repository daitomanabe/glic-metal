...
| Codec Glitch | 28 codec effects | `CVPixelBufferRef` | Stateful H.264 / HEVC / ProRes encode/decode and codec-history effects |
| Offline Packet Lab | 8 packet effects | video file | Isolated compressed-stream damage and salvage decode |
| Offline Syntax / Analysis Lab | 18 workflows | video file | Real codec cycles, reconstruction, analysis, and search |
...
`compat_realtime`のいずれでもありません。入力は`CVPixelBufferRef`で、28種類の
...
`CVPixelBufferRef` input and exposes the 28 effects documented in
